...
public class Test_comment {
    ...
    MockOrder c=new MockOrder("025", 1, 155, 3, true, "three", 307, time4, time5, time1, "", 70, time3);
	@Test
	public void test() {
		...
		order.comment("just so so", item3.getorder());
		assertEquals(item3.getorder().getComment(),"just so so");
	}
